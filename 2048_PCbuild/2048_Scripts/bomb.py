...
block_tile_row_index = random.randint(0, 3)
block_tile_col_index = random.randint(0, 3)
board_values[block_tile_row_index][block_tile_col_index] = 1
game_over = False
spawn_new = True
init_count = 0
direction = ''
...
def take_turn(direction, board):
    ...
    merged[block_tile_row_index][block_tile_col_index] = True
    if direction == 'UP':
        ...
        for j in range(4):
            ...
            if board[i][j] != 1:
                for k in range(i + 1):
                    ...
                    if board[3 - k][j] == 1:
                        shift = 0
                if shift > 0 and board[2 - i + shift][j] != 1 and board[2 - i][j] != 1:
        ...
        for j in range(4):
            ...
            if board[i][j] != 1:
                for k in range(j):
                    ...
                    if board[i][3 - k] == 1:
                        shift = 0
                if shift > 0 and board[i][3 - j + shift] != 1 and board[i][3 - j] != 1:
...
def draw_pieces(board):
    for i in range(4):
        for j in range(4):
            # print(board[i][j], end=' ')
            if board[i][j] == 1:
                pygame.draw.rect(screen, (255, 0, 0), [20 + 95 * j, 20 + 95 * i, 75, 75], 0, 5)
                continue
            value = board[i][j]
            if value > 8:
...
async def main():
    global colors, board_values, game_over, spawn_new, init_count, direction,\
         score, file, init_high, high_score, block_tile_row_index, block_tile_col_index
    run = True
    global color
    while run:
